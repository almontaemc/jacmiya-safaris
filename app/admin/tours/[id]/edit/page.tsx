...
import { getTours, updateTour, getExchangeRate, setExchangeRate } from "@/lib/adminStore";
import type { AdminTour, ItineraryDay } from "@/types/admin";
...
export default function EditTour({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    if (found) {
      ...
      setIncludes(found.includes);
      setItinerary(found.itinerary ?? []);
      if (found.image && !found.image.startsWith("http")) setImagePreview(found.image);
    }
  }, [id]);
  ...
  function handleDuration(text: string) {
    ...
    setFormState((p) => ({ ...p, duration: text, days: found?.days ?? p.days }));
  }
  function handleKSH(v: number) {
    setFormState((p) => ({ ...p, priceksh: v, priceusd: rate > 0 ? Math.round(v / rate) : p.priceusd }));
  }
  function handleUSD(v: number) {
    setFormState((p) => ({ ...p, priceusd: v, priceksh: Math.round(v * rate) }));
  }
  function saveRate() {
    setExchangeRate(rate);
    setRateSaved(true);
    setTimeout(() => setRateSaved(false), 2000);
    if (form.priceksh > 0) setFormState((p) => ({ ...p, priceusd: rate > 0 ? Math.round(p.priceksh / rate) : p.priceusd }));
  }
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!form.image) { alert("Please upload or enter a tour image."); return; }
    setSaving(true);
    updateTour(Number(id), { ...form, highlights, includes, itinerary });
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-gray-500 text-sm truncate max-w-xs">{tour.title}</p>
        </div>
      </div>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
          <div className="flex-1">
            ...
            <p className="text-xs text-amber-700">Change to auto-convert prices between KSH and USD. Saved globally.</p>
          </div>
          <div className="flex items-center gap-2 flex-shrink-0">
            <span className="text-sm font-semibold text-amber-800">1 USD =</span>
            <input type="number" value={rate} onChange={(e) => setRate(Number(e.target.value))}
              className="w-24 border border-amber-300 rounded-xl px-3 py-2 text-sm text-center font-bold focus:outline-none bg-white" />
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <Card title="Basic Info">
          <div className="space-y-4">
            <div>
              ...
              <input className={inputCls} value={form.title} onChange={(e) => setF("title", e.target.value)} required />
            </div>
            <div>
              <label className={labelCls}>Short Description</label>
              <textarea rows={2} className={`${inputCls} resize-none`} value={form.description} onChange={(e) => setF("description", e.target.value)} />
            </div>
        ...
        <Card title="Pricing">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="number" min={0} className={inputCls} value={form.priceksh} onChange={(e) => handleKSH(Number(e.target.value))} required />
              <p className="text-xs text-gray-400 mt-1">Changing KSH auto-sets USD</p>
            </div>
            <div>
              <label className={labelCls}>Price (USD) *</label>
              <input type="number" min={0} className={inputCls} value={form.priceusd} onChange={(e) => handleUSD(Number(e.target.value))} required />
          ...
          <div className="space-y-3">
            <div onClick={() => fileRef.current?.click()}
              ...
              {imagePreview ? (
                ...
                // eslint-disable-next-line @next/next/no-img-element
                <img src={form.image} alt="Current" className="h-44 object-cover rounded-xl mx-auto shadow-sm" />
              ) : (
            ...
            <div className="flex items-center gap-2 text-xs text-gray-400">
              <div className="flex-1 h-px bg-gray-100" /><span>or paste URL</span><div className="flex-1 h-px bg-gray-100" />
            </div>
            <input className={inputCls} value={form.image.startsWith("data:") ? "" : form.image}
              onChange={(e) => { setImagePreview(""); setF("image", e.target.value); }} placeholder="https://example.com/image.jpg" />
          </div>
        </Card>

        <Card title="Tour Highlights">
          <p className="text-xs text-gray-500 mb-3">Toggle to select/deselect. Add custom highlights below.</p>
        ...
        <Card title="Day-by-Day Itinerary">
          <p className="text-xs text-gray-500 mb-4">Edit the day-by-day breakdown shown on the public tour detail page.</p>
          <div className="space-y-4">
            {itinerary.map((day, i) => (
              <div key={i} className="bg-gray-50 rounded-xl p-4 border border-gray-100">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button type="button" onClick={() => removeDay(i)} className="text-gray-400 hover:text-red-500 transition-colors p-1"><Trash2 className="w-4 h-4" /></button>
                </div>
                <div className="space-y-2">
                  <input className={inputCls} placeholder={`Day ${day.day} title`} value={day.title} onChange={(e) => updateDay(i, "title", e.target.value)} />
                  <textarea rows={2} className={`${inputCls} resize-none`} placeholder="Day description…" value={day.description} onChange={(e) => updateDay(i, "description", e.target.value)} />
                  <div className="grid grid-cols-2 gap-2">
                    <input className={inputCls} placeholder="Meals" value={day.meals ?? ""} onChange={(e) => updateDay(i, "meals", e.target.value)} />
                    <input className={inputCls} placeholder="Accommodation" value={day.accommodation ?? ""} onChange={(e) => updateDay(i, "accommodation", e.target.value)} />
                  </div>
                </div>
              </div>
            ))}
